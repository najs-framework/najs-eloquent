...
function test_syntax() {
    ...
    function test_collection() {
        return __awaiter(this, void 0, void 0, function* () {
            const value_child = yield Child.select().all();
            value_child.map(item => item.child_virtual_attribute).toArray();
            const value_parent = yield Child.select().all();
            value_parent
                .map(function (item) {
    ...
    function test_eloquent() {
        return __awaiter(this, void 0, void 0, function* () {
            const eloquent = (yield Child.select().all()).first();
            ...
            use(date_parent);
            eloquent.save();
            const data = yield Child.where('test', 1).get();
            data.reduce(function () { }).all();
            const result = yield Child.find(1);
            result.parentMethod();
        });
    }
    use(test_eloquent);
