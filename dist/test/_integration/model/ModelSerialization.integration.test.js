...
describe('Model Serialization Feature', function () {
    describe('No Setting', function () {
        class ModelNoSetting extends index_1.Model {
            constructor() {
                super(...arguments);
                this.fillable = ['a', 'b', 'c', 'd'];
            }
            getClassName() {
                return 'ModelNoSetting';
            }
        }
        index_1.Model.register(ModelNoSetting);
        function makeModel() {
        ...
        it('.makeVisible() should work with empty visible setting', function () {
            expect(makeModel()
                .makeVisible(['a', 'b'])
                .toObject()).toEqual({ a: 1, b: 2, c: 3, d: 4 });
        });
